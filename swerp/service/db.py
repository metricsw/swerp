...
import swerp
from swerp import SUPERUSER_ID
from swerp.exceptions import AccessDenied
import swerp.release
import swerp.sql_db
import swerp.tools
from swerp.sql_db import db_connect
from swerp.release import version_info
from swerp.tools import pycompat
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not swerp.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and swerp.tools.config.verify_admin_password(passwd):
        return True
    raise swerp.exceptions.AccessDenied()

# This should be moved to swerp.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = swerp.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            swerp.modules.db.initialize(cr)
            swerp.tools.config['load_language'] = lang
            cr.commit()

        registry = swerp.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = swerp.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = swerp.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = swerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = swerp.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    swerp.sql_db.close_db(db_original_name)
    db = swerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = swerp.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = swerp.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = swerp.tools.config.filestore(db_original_name)
    to_fs = swerp.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    swerp.modules.registry.Registry.delete(db_name)
    swerp.sql_db.close_db(db_name)
    ...
    fs = swerp.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'swerp_dump': '1',
        'db_name': cr.dbname,
        'version': swerp.release.version,
        'version_info': swerp.release.version_info,
        'major_version': swerp.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with swerp.tools.osutil.tempdir() as dump_dir:
            filestore = swerp.tools.config.filestore(db_name)
            ...
            swerp.tools.exec_pg_command(*cmd)
            if stream:
                swerp.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                swerp.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = swerp.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with swerp.tools.osutil.tempdir() as dump_dir:
        ...
        if swerp.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = swerp.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = swerp.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if swerp.tools.config['unaccent']:
                try:
                    with cr.savepoint():
...
def exp_rename(old_name, new_name):
    swerp.modules.registry.Registry.delete(old_name)
    swerp.sql_db.close_db(old_name)
    ...
    old_fs = swerp.tools.config.filestore(old_name)
    new_fs = swerp.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    swerp.tools.config.set_admin_password(new_password)
    swerp.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        swerp.tools.config['update']['base'] = True
        swerp.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@swerp.tools.mute_logger('swerp.sql_db')
...
def list_dbs(force=False):
    if not swerp.tools.config['list_db'] and not force:
        raise swerp.exceptions.AccessDenied()

    if not swerp.tools.config['dbfilter'] and swerp.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, swerp will not
        ...
        res = sorted(db.strip() for db in swerp.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [swerp.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of swerp
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if swerp.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        swerp.sql_db.close_db(database_name)
...
def exp_list_lang():
    return swerp.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(swerp.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
