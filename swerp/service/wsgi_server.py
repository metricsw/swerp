...
# User code must use the exceptions defined in ``swerp.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, swerp.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, swerp.tools.ustr(e.name))
    ...
    elif isinstance(e, swerp.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = swerp.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, swerp.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(swerp.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(swerp.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, swerp is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(swerp.tools.ustr(value), write)
...
def application_unproxied(environ, start_response):
    ...
    # swerp.service.web_services.objects_proxy.dispatch().
    ...
    with swerp.api.Environment.manage():
        result = swerp.http.root(environ, start_response)
