...
import swerp
from swerp.modules import get_modules
from swerp.modules.module import run_unit_tests, get_test_modules
from swerp.modules.registry import Registry
from swerp.release import nt_service_name
from swerp.tools import config
from swerp.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'swerp.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The swerp_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("swerp_MAX_HTTP_THREADS")
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(swerp, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in swerp.modules.module.ad_paths:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="swerp.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from swerp.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = swerp.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(swerp.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="swerp.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="swerp.service.httpd")
        t.setDaemon(True)
        t.start()
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('swerp: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(swerp.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The swerp_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("swerp_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = swerp.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('swerp.netsvc.rpc.request')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(swerp.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            swerp.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if swerp.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    stream = swerp.modules.module.TestStream()
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_file']:
                ...
                with swerp.api.Environment.manage():
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = swerp.sql_db.sql_counter
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, swerp.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the swerp http server and cron processor.
    """
    global server

    load_server_wide_modules()
    swerp.service.wsgi_server._patch_xmlrpc_marshaller()

    if swerp.evented:
        server = GeventServer(swerp.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(swerp.service.wsgi_server.application)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which swerp is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(swerp.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not swerp.evented:
        if inotify:
    ...
    if getattr(swerp, 'phoenix', False):
        _reexec()
