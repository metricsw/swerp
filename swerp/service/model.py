...
import swerp
from swerp.exceptions import UserError, ValidationError, QWebException
from swerp.models import check_method_name
from swerp.tools.translate import translate
...
def dispatch(method, params):
    ...
    registry = swerp.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = swerp.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if swerp.registry(dbname)._init and not swerp.tools.config['test_enable']:
                    raise swerp.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    swerp.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = swerp.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    result = swerp.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with swerp.registry(db).cursor() as cr:
