...
# ----------------------------------------------------------
# swerp must be a namespace package for swerp.addons to become one too
# https://packaging.python.org/guides/packaging-namespace-packages/
# ----------------------------------------------------------
__path__ = __import__('pkgutil').extend_path(__path__, __name__)

# python 2 is no supported, ensure py version is >= 3.5
import sys
assert sys.version_info > (3, 5), "Python 2 detected, Swerp requires Python >= 3.5 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in swerp/debian/copyright)
        while 1:
            state = conn.poll()
