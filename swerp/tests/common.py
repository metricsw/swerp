# -*- coding: utf-8 -*-
"""
The module :mod:`swerp.tests.common` provides unittest test cases and a few
...
from swerp.models import BaseModel
from swerp.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from swerp.tools import pycompat
from swerp.tools import single_email_re
from swerp.tools.misc import find_in_path
from swerp.tools.safe_eval import safe_eval
...
import swerp
import pprint
from swerp import api
from swerp.service import security



_logger = logging.getLogger(__name__)

# The swerp library is supposed already configured.
ADDONS_PATH = swerp.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = swerp.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = swerp.SUPERUSER_ID


def get_db_name():
    db = swerp.tools.config['db_name']
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most swerp operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('swerp.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    ...
    longMessage = True      # more verbose error message by default: https://www.swerp.it/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~swerp.models.BaseModel`
        """
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/swerp/addons/" in filename:
                        filename = filename.rsplit("/swerp/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = swerp.registry(get_db_name())
        #: current transaction's cursor
        self.cr = self.cursor()
        #: :class:`~swerp.api.Environment` for the current test case
        self.env = api.Environment(self.cr, swerp.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.registry = swerp.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.env = api.Environment(cls.cr, swerp.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_swerp')
    ...
    def _get_shotname(self, prefix, ext):
        ...
        base_file = os.path.splitext(swerp.tools.config['logfile'])[0]
        name = '%s_%s_%s.%s' % (base_file, prefix, timestamp, ext)
        return name

    def take_screenshot(self, prefix='failed'):
        if not swerp.tools.config['logfile']:
    ...
    def _save_screencast(self, prefix='failed'):
        ...
        if not swerp.tools.config['logfile']:
            ...
            return None
        sdir = tempfile.mkdtemp(suffix='_chrome_swerp_screencast')
        nb = 0
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = swerp.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        swerp.http.root.session_store.save(self.session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('swerp.service.http.request.'):
                join_retry_count = 10
    ...
    def authenticate(self, user, password):
        ...
        if user is None:
            if self.session:
                swerp.http.root.session_store.delete(self.session)
        ...
        swerp.http.root.session_store.save(session)
        if self.browser:
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~swerp.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~swerp.fields.Many2many` fields, the field itself is a
    :class:`~swerp.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~swerp.fields.One2many` are reified as
    :class:`~swerp.tests.common.O2MProxy`.

    Because the :class:`~swerp.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~swerp.tests.common.O2MProxy.new` and
    :meth:`~swerp.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: swerp.models.Model
    ...
    :type view: int | str | swerp.model.Model
    ...
    def _init_from_defaults(self, model):
        ...
        def cleanup(k, v):
            if fields[k]['type'] == 'one2many':
                ...
                return swerp.fields.Datetime.to_string(v)
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~swerp.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~swerp.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, descr in fields.items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = swerp.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = swerp.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return swerp.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return swerp.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from swerp.tests.common have a test_tags
