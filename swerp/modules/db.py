...
def is_initialized(cr):
    ...
    """
    return swerp.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = swerp.modules.get_module_resource('base', 'data', 'base_data.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with swerp.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())

    for i in swerp.modules.get_modules():
        mod_path = swerp.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = swerp.modules.load_information_from_description_file(i)
