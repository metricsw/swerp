...
import swerp
import swerp.modules.db
import swerp.modules.graph
import swerp.modules.migration
import swerp.modules.registry
import swerp.tools as tools

from swerp import api, SUPERUSER_ID
from swerp.modules.module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('swerp.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            swerp.registry(cr.dbname).clear_caches()
    ...
    migrations = swerp.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        if new_install:
            py_module = sys.modules['swerp.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = swerp.tools.config["overwrite_existing_translations"]
            ...
            if demo_loaded:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(swerp.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, swerp.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        if not swerp.modules.db.is_initialized(cr):
        ...
        # swerp.modules.registry.Registry.new().
        registry = swerp.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['swerp.addons.%s' % (pkg.name,)]
                ...
                registry = swerp.modules.registry.Registry.new(
