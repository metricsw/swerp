...
import swerp
import swerp.tools as tools
import swerp.release as release
from swerp import SUPERUSER_ID, api
...
class AddonsHook(object):
    """ Makes modules accessible through openerp.addons.* and swerp.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('swerp.addons.', 'openerp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        swerp_name = re.sub(r'^openerp.addons.(\w+)$', r'swerp.addons.\g<1>', name)
        openerp_name = re.sub(r'^swerp.addons.(\w+)$', r'openerp.addons.\g<1>', swerp_name)

        assert swerp_name not in sys.modules
        ...
        # create empty swerp.addons.* module, set name
        new_mod = types.ModuleType(swerp_name)
        new_mod.__loader__ = self

        # module top-level can only be a package
        assert type_ == imp.PKG_DIRECTORY, "swerp addon top-level must be a package"
        modfile = opj(path, '__init__.py')
        new_mod.__file__ = modfile
        new_mod.__path__ = [path]
        new_mod.__package__ = swerp_name

        # both base and alias should be in sys.modules to handle recursive and
        # corecursive situations
        sys.modules[swerp_name] = sys.modules[openerp_name] = new_mod
        ...
        setattr(swerp.addons, addon_name, new_mod)
...
class SwerpHook(object):
    """ Makes swerp package also available as openerp """
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'swerp\g<1>', name)
...
def get_module_filetree(module, dir='.'):
    ...
    files = swerp.tools.osutil.listdir(path, True)
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('swerp.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['swerp.addons.' + module_name], info['post_load'])()
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('swerp.addons', module)

    try:
        importlib.import_module('swerp.addons.base.maintenance.migrations.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('swerp.addons.base.maintenance.migrations', module)
...
class TestStream(object):
    def __init__(self, logger_name='swerp.tests'):
...
def run_unit_tests(module_name, dbname, position='at_install'):
    ...
    """
    global current_test
    from swerp.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, swerp.sql_db.sql_counter - t0_sql)
