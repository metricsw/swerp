...
# *broken*
NO_POSTMORTEM = (swerp.osv.orm.except_orm,
                 swerp.exceptions.AccessError,
                 swerp.exceptions.ValidationError,
                 swerp.exceptions.MissingError,
                 swerp.exceptions.AccessDenied,
                 swerp.exceptions.Warning,
                 swerp.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                swerp.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = swerp.service.common.dispatch
        elif service_name == 'db':
            dispatch = swerp.service.db.dispatch
        elif service_name == 'object':
            dispatch = swerp.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                swerp.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                swerp.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except swerp.exceptions.DeferredException as e:
        _logger.exception(swerp.tools.exception_to_unicode(e))
        swerp.tools.debugger.post_mortem(swerp.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(swerp.tools.exception_to_unicode(e))
        swerp.tools.debugger.post_mortem(swerp.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all Swerp Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in swerp.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~swerp.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~swerp.api.Environment` bound to current request. """
        if self._env is None:
            self._env = swerp.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            swerp.tools.debugger.post_mortem(
                swerp.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Swerp implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <swerp.http.WebRequest.csrf_token`, the
          :data:`~swerp.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          Swerp) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (swerp.exceptions.Warning, SessionExpiredException,
                                          swerp.exceptions.except_orm, werkzeug.exceptions.NotFound)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "Swerp Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "Swerp Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "Swerp Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, swerp.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, swerp.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, swerp.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, swerp.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, swerp.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, swerp.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, swerp.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, swerp.exceptions.except_orm):
...
Swerp URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.swerp.it for
more details.

* if this endpoint is accessed through Swerp via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["swerp", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('swerp.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = swerp.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = swerp.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or swerp.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = swerp.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        return routing_map([''] + swerp.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in swerp.modules.module.ad_paths:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        swerp.registry(db).check_signaling()
                        with swerp.tools.mute_logger('swerp.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = swerp.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if swerp.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = swerp.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif swerp.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Swerp will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in swerp.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = swerp.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = swerp.tools.ustr(filename)
