...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/swerp" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -df
cp -a /home/pi/swerp/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/swerp/* /home/pi/swerp/
rm -r /home/pi/swerp/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service swerp restart) &
