...
copy_raspbian () {
    ...
    wget -c 'http://nightly.swerp.it/master/posbox/iotbox/iotbox-latest.zip' -O raspbian/iotbox-latest.zip
    wget -c 'http://nightly.swerp.it/master/posbox/iotbox/SHA1SUMS.txt' -O raspbian/SHA1SUMS.txt
    cd raspbian/
...
copy_iot () {
    ...
    # Modify startup
    mkdir -v swerp
    mount -v "${PART_IOTBOX_ROOT}" swerp
    cp -v /home/pi/upgrade.sh swerp/home/pi/
    NBR_LIGNE=$(sed -n -e '$=' swerp/etc/rc.local)
    sed -ie "${NBR_LIGNE}"'i\. /home/pi/upgrade.sh; clean_local' swerp/etc/rc.local
    find /home/pi/config -maxdepth 1 -type f ! -name ".*" -exec cp {} swerp/home/pi/ \;
