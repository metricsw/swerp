swerp.define('partner.autocomplete.core', function (require) {
'use strict';
...
return {
    _dropPreviousSwerp: new concurrency.DropPrevious(),
    ...
    autocomplete: function (value) {
        ...
        var swerpPromise = this._getSwerpSuggestions(value, isVAT).then(function (suggestions){
            swerpSuggestions = suggestions;
        });
        ...
        var concatResults = function () {
            // Add Clearbit result with Swerp result (with unique domain)
            if (clearbitSuggestions && clearbitSuggestions.length) {
                var websites = swerpSuggestions.map(function (suggestion) {
                ...
                clearbitSuggestions.forEach(function (suggestion) {
                    if (websites.indexOf(suggestion.domain) < 0) {
                        ...
                        swerpSuggestions.push(suggestion);
                    }
                });
            }

            swerpSuggestions = _.filter(swerpSuggestions, function (suggestion) {
                return !suggestion.ignored;
            });
            _.each(swerpSuggestions, function(suggestion){
              delete suggestion.ignored;
            });
            return def.resolve(swerpSuggestions);
        };

        this._whenAll([swerpPromise, clearbitPromise]).then(concatResults, concatResults);
    ...
    /**
     * Use Swerp Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Deferred}
     * @private
     */
    _getSwerpSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousSwerp.add(def);
    },
    /**
