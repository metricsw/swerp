...
from swerp import _, api, fields, models, modules, SUPERUSER_ID, tools
from swerp.exceptions import UserError, AccessError
from swerp.osv import expression
from swerp.tools import groupby, formataddr
...
class Message(models.Model):
    ...
    def _notify_compute_recipients(self, record, msg_vals):
        ...
        for pid, cid, active, pshare, ctype, notif, groups in res:
            ...
            if pid:
                if active is False:
                    # avoid to notify inactive partner by email (swerpbot)
                    continue
