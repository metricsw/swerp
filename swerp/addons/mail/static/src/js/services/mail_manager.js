swerp.define('mail.Manager', function (require) {
"use strict";
...
var MailManager =  AbstractService.extend({
    ...
    /**
     * Get the SwerpBot ID, which is the default authorID for transient messages
     *
     * @returns {string}
     */
    getSwerpbotID: function () {
            ...
            if (thread) {
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        if (!self.call('bus_service', 'isSwerpFocused') || !query.isVisible) {
    ...
    _initializeInternalState: function () {
        ...
        // # of message received when swerp is out of focus
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isSwerpFocused')) {
            // no need to notify
            return;
        }
