...
import swerp
from swerp.addons.payment.models.payment_acquirer import ValidationError
from swerp.addons.payment.tests.common import PaymentAcquirerCommon
from swerp.addons.payment_authorize.controllers.main import AuthorizeController
...
@swerp.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('swerp.addons.payment_authorize.models.payment', 'ValidationError')
...
@swerp.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeForm(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on swerp/swerp + create sandbox account
        authorize.write({
