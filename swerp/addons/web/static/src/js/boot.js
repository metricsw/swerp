...
(function() {
    ...
    var swerp = window.swerp = window.swerp || {};
    _.extend(swerp, {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: swerp.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = swerp.__DEBUG__.get_dependencies( job.name );
                ...
                missing = swerp.__DEBUG__.get_missing_jobs();
                failed = swerp.__DEBUG__.get_failed_jobs();
                var unloaded = _.filter(debug_jobs, function (job) { return job.missing; });

                if (swerp.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                    ...
                    if (swerp.debug && !_.isEmpty(debug_jobs)) {
                        log('Debug:                   ', debug_jobs);
                    }
                }
            }
            swerp.__DEBUG__.js_modules = {
                missing: missing,
                failed: _.pluck(failed, 'name'),
            };
            swerp.__DEBUG__.didLogInfo.resolve();
        },
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            swerp.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
