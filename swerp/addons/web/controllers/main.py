...
import swerp
import swerp.modules.registry
from swerp.api import call_kw, Environment
from swerp.modules import get_resource_path
from swerp.tools import crop_image, topological_sort, html_escape, pycompat
from swerp.tools.mimetypes import guess_mimetype
from swerp.tools.translate import _
from swerp.tools.misc import str2bool, xlwt, file_open
from swerp.tools.safe_eval import safe_eval
from swerp import http
from swerp.http import content_disposition, dispatch_rpc, request, \
    serialize_exception as _serialize_exception, Response
from swerp.exceptions import AccessError, UserError, AccessDenied
from swerp.models import check_method_name
from swerp.service import db, security
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('swerp.addons.web', "views")
...
def module_installed_bypass_session(dbname):
    try:
        registry = swerp.registry(dbname)
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, swerp.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = swerp.conf.server_wide_modules or []
    serverside = ['base', 'web']
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("swerp.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, swerp has weird dealings in relation to
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = swerp.SUPERUSER_ID
        ...
        if request.httprequest.method == 'POST':
            ...
            except swerp.exceptions.AccessDenied as e:
                request.uid = old_uid
                if e.args == swerp.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = swerp.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def version_info(self):
        return swerp.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = swerp.tools.config.verify_admin_password('admin')
        d['list_db'] = swerp.tools.config['list_db']
        d['langs'] = swerp.service.db.exp_list_lang()
        d['countries'] = swerp.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = swerp.service.db.list_db_incompatible(d['databases'])
        except swerp.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            swerp.service.db.check_super(master_pwd)
            ...
            dump_stream = swerp.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=swerp.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.swerp.it/oauth2/auth?' + werkzeug.url_encode(params)
        ...
        if not content:
            ...
            if not (width or height):
                ...
                if suffix in ('small', 'medium', 'big'):
                    content = getattr(swerp.tools, 'image_resize_image_%s' % suffix)(content)
        ...
        if crop and (width or height):
            ...
            if not upper_limit:
                ...
                if height > 500:
                    height = 500
            try:
                content = swerp.tools.image_resize_image(base64_source=content, size=(width or None, height or None),
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = swerp.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: (field[0] not in ['id', '.id', 'display_name', 'name'], swerp.tools.ustr(field[1].get('string', ''))))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from swerp's export data to whatever the
