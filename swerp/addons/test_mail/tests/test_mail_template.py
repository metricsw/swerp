...
from swerp.tools import mute_logger, DEFAULT_SERVER_DATETIME_FORMAT
...
class TestMailTemplate(BaseFunctionalTest, MockEmails, TestRecipients):
    ...
    def test_template_send_email_translations_with_notif_layout(self):
        ...
        ARCH = '<t t-name="test_mail.test_notification_template">%s</t>'
        ...
        mail = self.env['mail.mail'].browse(mail_id)
        self.assertEqual(mail.body_html, TEXT_FR)
    ...
    #     with patch('swerp.addons.mail.tests.test_mail_template.datetime', wraps=datetime) as mock_datetime:
    ...
    def test_create_partner_from_tracking_multicompany(self):
        ...
        with patch('swerp.addons.mail.models.mail_thread.MailThread._message_track_post_template', patched_message_track_post_template):
