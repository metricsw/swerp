...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a Swerp DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of swerp DB
        2 - token. This token will be compared to the token of Swerp. He have 1 hour lifetime
        """
...
class ConnectionManager(Thread):
    ...
    def run(self):
        if not helpers.get_swerp_server_url():
    ...
    def _connect_box(self):
        ...
        req = requests.post('https://iot-proxy.swerp.it/swerp-enterprise/iot/connect-box', json=data, verify=False)
...
class Manager(Thread):
    ...
    def load_drivers(self):
        """
        This method loads local files: 'swerp/addons/hw_drivers/drivers'
    ...
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Swerp database
        """
        server = helpers.get_swerp_server_url()
        if server:
            subject = helpers.read_file_first_line('swerp-subject.conf')
            if subject:
            ...
            _logger.warning('Swerp server not set')
    ...
    def run(self):
        """
        Thread that will check connected/disconnected device, load drivers if needed and contact the swerp server with the updates
        """
