...
from swerp import fields as swerp_fields, tools, _
from swerp.exceptions import ValidationError, AccessError, MissingError, UserError
from swerp.http import content_disposition, Controller, request, route
from swerp.tools import consteq
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': swerp_fields.Date.to_string(swerp_fields.Date.from_string(date_begin)),
                'date_end': swerp_fields.Date.to_string(swerp_fields.Date.from_string(date_end)),
                'name': label,
