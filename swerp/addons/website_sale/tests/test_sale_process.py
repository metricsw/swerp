...
from swerp.addons.website_sale.controllers.main import WebsiteSale
from swerp.addons.website.tools import MockRequest


@swerp.tests.tagged('post_install', '-at_install')
class TestUi(swerp.tests.HttpCase):
    def test_01_admin_shop_tour(self):
        self.phantom_js("/", "swerp.__DEBUG__.services['web_tour.tour'].run('shop')", "swerp.__DEBUG__.services['web_tour.tour'].tours.shop.ready", login="admin")

    def test_02_admin_checkout(self):
        self.phantom_js("/", "swerp.__DEBUG__.services['web_tour.tour'].run('shop_buy_product')", "swerp.__DEBUG__.services['web_tour.tour'].tours.shop_buy_product.ready", login="admin")

    def test_03_demo_checkout(self):
        self.phantom_js("/", "swerp.__DEBUG__.services['web_tour.tour'].run('shop_buy_product')", "swerp.__DEBUG__.services['web_tour.tour'].tours.shop_buy_product.ready", login="demo")


@swerp.tests.tagged('post_install', '-at_install')
class TestWebsiteSaleCheckoutAddress(swerp.tests.TransactionCase):
