swerp.define('web_editor.backend', function (require) {
'use strict';
...
var FieldTextHtmlSimple = basic_fields.DebouncedField.extend(TranslatableFieldMixin, {
     ...
     *
     * @private
     * @returns {Array} "ir.attachment" swerp domain.
     */
...
var FieldTextHtml = AbstractField.extend({
    ...
    start: function () {
        ...
        window.swerp[this.callback+"_editor"] = function (EditorBar) {
            setTimeout(function () {
                self.on_editor_loaded(EditorBar);
            },0);
        };
        window.swerp[this.callback+"_content"] = function () {
            self.on_content_loaded();
        };
        window.swerp[this.callback+"_updown"] = null;
        window.swerp[this.callback+"_downup"] = function () {
            self.resize();
        };
    ...
    on_editor_loaded: function (EditorBar) {
        ...
        if (this.value && window.swerp[self.callback+"_updown"] && !(this.$content.html()||"").length) {
            this.render();
        }
    ...
    render: function () {
        ...
        if (this.mode === "edit") {
            if (window.swerp[this.callback+"_updown"]) {
                // FIXME
                // window.swerp[this.callback+"_updown"](value, this.view.get_fields_values(), this.name);
                this.resize();
            }
        } else {
    ...
    destroy: function () {
        ...
        delete window.swerp[this.callback+"_editor"];
        delete window.swerp[this.callback+"_content"];
        delete window.swerp[this.callback+"_updown"];
        delete window.swerp[this.callback+"_downup"];
    },
