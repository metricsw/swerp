swerp.define('mail_bot.systray.MessagingMenu', function (require) {
"use strict";
...
return MessagingMenu.include({
     ...
     * messaging menu (e.g. 'SwerpBot has a request')
     *
     * @override
     * @private
     ...
     * SwerpBot, and the SwerpBot influences the counter by 1 when it requests
    ...
    _handleResponseNotificationPermission: function (value) {
        ...
        if (value !== 'granted') {
            this.call('bus_service', 'sendNotification', _t('Permission denied'),
                _t('Swerp will not have the permission to send native notifications on this device.'));
        } else {
            this.call('bus_service', 'sendNotification', _t('Yay, push notifications are enabled!'),
                _t('Swerp has now the permission to send you native notifications on this device.'));
        }
        this._updateCounter();
    },
    /**
    ...
    /**
     * Override so that it handles preview related to SwerpBot
     *
     * @override
     * @private
     * @param {MouseEvent} ev
