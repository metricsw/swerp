...
class Shell(Command):
    """Start swerp in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        swerp.cli.server.report_configuration()
        swerp.service.server.start(preload=[], stop=True)
        ...
        with swerp.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = swerp.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = swerp.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
