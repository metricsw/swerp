...
__author__ = swerp.release.author
__version__ = swerp.release.version

# Also use the `swerp` logger for the main script.
_logger = logging.getLogger('swerp')
...
def report_configuration():
    ...
    """
    config = swerp.tools.config
    _logger.info("swerp version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', swerp.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = swerp.tools.config
    if not swerp.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = swerp.modules.registry.Registry.new(dbname)
        with swerp.api.Environment.manage():
            with registry.cursor() as cr:
                swerp.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with swerp.api.Environment.manage():
        with registry.cursor() as cr:
            swerp.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    swerp.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                swerp.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except swerp.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        swerp.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = swerp.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the swerp server (default command)"""
    def run(self, args):
        main(args)
