...
from swerp.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if swerp.tools.config['db_host']:
        env['PGHOST'] = swerp.tools.config['db_host']
    if swerp.tools.config['db_port']:
        env['PGPORT'] = str(swerp.tools.config['db_port'])
    if swerp.tools.config['db_user']:
        env['PGUSER'] = swerp.tools.config['db_user']
    if swerp.tools.config['db_password']:
        env['PGPASSWORD'] = swerp.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    ...
    """
    import swerp.modules as addons
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = swerp.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('swerp.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('swerp.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, pycompat.string_types):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = swerp.fields.Datetime.from_string(value)
            value = swerp.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = swerp.fields.Datetime.from_string(value)
