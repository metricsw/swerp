...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('swerp.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
DEFAULT_LOG_CONFIGURATION = [
    'swerp.http.rpc.request:INFO',
    'swerp.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['swerp:DEBUG', 'swerp.sql_db:INFO', 'swerp.http.rpc:DEBUG'],
    'debug_rpc': ['swerp:DEBUG', 'swerp.sql_db:INFO', 'swerp.http.rpc.request:DEBUG'],
    'debug': ['swerp:DEBUG', 'swerp.sql_db:INFO'],
    'debug_sql': ['swerp.sql_db:DEBUG'],
    'info': [],
    'warn': ['swerp:WARNING', 'werkzeug:WARNING'],
    'error': ['swerp:ERROR', 'werkzeug:ERROR'],
    'critical': ['swerp:CRITICAL', 'werkzeug:CRITICAL'],
}
