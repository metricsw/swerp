...
from swerp.tools import frozendict, classproperty, StackMap, pycompat
from swerp.exceptions import CacheMiss
...
def constrains(*args):
    ...
    Should raise :class:`~swerp.exceptions.ValidationError` if the
    validation failed.
...
def one(method):
    """ Decorate a record-style method where ``self`` is expected to be a
        ...
        .. deprecated:: 9.0
            ...
            is a single record with :meth:`~swerp.models.Model.ensure_one`.
    """
...
from swerp.exceptions import UserError, AccessError, MissingError
