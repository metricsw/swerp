...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (OpenERP) module name can be in the ``swerp.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``swerp.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['swerp', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """ Base class for Swerp models.

    Swerp models are created by inheriting:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = swerp.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or swerp.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from swerp.fields import Char, Integer
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~swerp.fields.Integer`,
          :class:`~swerp.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~swerp.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~swerp.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~swerp.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~swerp.fields.Date` and
              :class:`~swerp.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`swerp.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`swerp.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~swerp.fields.One2many` and
          :class:`~swerp.fields.Many2many` use a special "commands" format to
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~swerp.api.Environment`
        """
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted swerp models.

    swerp models are created by inheriting from this class::
